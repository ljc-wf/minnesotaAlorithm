...
#define ISO_LENGTH1  BEAT_MS50 // rahul - needs to be checked ???
#define ISO_LENGTH2	BEAT_MS80
#define ISO_LIMIT	20 // rahul - needs to be checked - will it be 20 or 8 ???

// added rahul start

extern int central_point_QRS;


#define ISO_LENGTH3  BEAT_MS20
#define ISO_LENGTH2	BEAT_MS80   //// 12
//#define ISO_LIMIT	8  /// in the past it's 15
#define abs(A)	((A) >= 0 ? (A) : -(A))

int g_Iso_point = 0;
int g_Iso_point_off = 0;
...
*/
	...
	*amp = maxV-minV ;
	...
	void AnalyzeQRS(int *beat, int *onset, int *offset)
	{
		int maxSlope = 0, maxSlopeI, minSlope = 0, minSlopeI;
		int maxV, minV ;
		int isoStart, isoEnd ;
		int slope, i ;
		int start_point = central_point_QRS;
		...
		for(i = start_point; (i < BEATLGTH) && (IsoCheck(&beat[i],ISO_LENGTH1) == 0); ++i) ;
		isoEnd = i ;
	
		// Find the maximum and minimum slopes on the
		// QRS complex.
	
		i = start_point-BEAT_MS110 ;
		maxSlope = maxSlope = beat[i] - beat[i-1] ;
		maxSlopeI = minSlopeI = i ;
	
		for(; i < start_point+BEAT_MS110; ++i)
		{
			slope = beat[i] - beat[i-1] ;
			if(slope > maxSlope)
			{
		...
		if(maxSlopeI < minSlopeI)
		{
			...
			// Search back from the maximum slope point for the QRS onset.
			int compared_slope = maxSlope >> 2;
			if (maxSlope > 80)
			{
				compared_slope = maxSlope >> 3;
			}
	
			for(i = maxSlopeI;
				(i > 0) ; --i) 
			{
				int dif = beat[i] - beat[i - 2];
				if ( abs(beat[i] - beat[i - 2]) < compared_slope*1.5 && abs((beat[i]-beat[i-1]) < compared_slope ))
				{
					break;
				}
			}
			*onset = i-1 ;
			////	 add at 7/26/2009
			if (*onset <= central_point_QRS - 12)
			{
				for(i = maxSlopeI;
					(i > 0) && ((beat[i]-beat[i-1]) > -compared_slope/*&& (beat[i] - beat[i - 3] > -compared_slope)*/); --i) ;
					*onset = i-1 ;
			}
			...
			if(i > *onset-INF_CHK_N)
			{
				for(;(i > 0) && ((beat[i]-beat[i-1]) > compared_slope); --i) ;
				*onset = i-1 ;
			}
			i = *onset+1 ;
			...
			//for(;(i > *onset-INF_CHK_N) && ((beat[i-1]-beat[i]) < compared_slope); --i);
			//if(i > *onset-INF_CHK_N)
			//{
			//	for(; (i > 0) && ((beat[i-1]-beat[i]) > compared_slope); --i) ;
			//	*onset = i-1 ;
			//}
		...
		{
			int compared_slope = minSlope >> 2;
			if (abs(minSlope) > 80)
			{
				compared_slope = minSlope >> 3;
			}
	
			// Search back from the minimum slope point for the QRS onset.
	
			for(i = minSlopeI;
				(i > 0) && ((beat[i]-beat[i-1]) < (compared_slope)); --i) ;
				*onset = i-1 ;
			///////////added in 7/26/2009
			if (*onset <= central_point_QRS - 12)
			{
			...
			for(; (i > *onset-INF_CHK_N) && ((beat[i]-beat[i-1]) >= (compared_slope)); --i) ;
			if(i > *onset-INF_CHK_N)
			{
				for(; (i > 0) && ((beat[i]-beat[i-1]) < (compared_slope));--i) ;
				*onset = i-1 ;
		...
		for(i = *onset; (i > 0) && (IsoCheck(&beat[i],ISO_LENGTH1) == 0); --i) ;	  ////	 ISO_LENGTH1 = 6;
		...
		if(i <= *onset - 10)
		{
			for(i = *onset; (i > 0) && (IsoCheck(&beat[i],ISO_LENGTH1) == 0); --i) ;
			isoStart = i + (ISO_LENGTH1 - 1) ;
			if (i <= *onset - 10)
			{
		//		i = i + 2;	 ///   because it's quite possible that it's the end of p wave, so go forward 5points
				if (i <= *onset - 10) // if still too small
				{
					for(i = *onset ; (i > 0) && (IsoCheck(&beat[i],ISO_LENGTH3) == 0); --i) ;
					isoStart = i + (ISO_LENGTH3 - 1) ;
					if (i <= *onset - 10)
					{
						g_Iso_point = *onset - 1;
					}
		...
		if(*onset < g_Iso_point) // ** 4/19 **
		{
			isoStart = *onset ;
			g_Iso_point = isoStart - ISO_LENGTH1/2;
		...
		else if(*onset-isoStart < BEAT_MS20)			   
			*onset = isoStart ;
		...
		//if (isoEnd > *offset + 8)
		//{
		//	g_Iso_point_off = *offset + 10;
		//}
		//else 
		//	g_Iso_point_off = isoEnd + 10;
		if (g_Iso_point < *onset - 13)
		{
			g_Iso_point = *onset - 13;
		}
		// Find the maximum and minimum values in the QRS.
